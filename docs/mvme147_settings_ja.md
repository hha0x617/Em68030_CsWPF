...
`em68030_CsWPF` の設定ダイアログ **MVME147** タブで変更を加えたとき、
エミュレータにどう適用されるかを説明します。同じ挙動は
`em68030_WinUI3Cpp` (C++ WinUI 3 スタンドアロン) と、プラグイン
フロントエンド `emfe_WinUI3Cpp` / `emfe_CsWPF` にも当てはまります —
設定層 (`EmulatorConfig` と apply 処理) が 4 フロントエンド共通
だからです。
