...
- bool 分岐でメソッド選択。Func<bool> デリゲートディスパッチは約 17% の性能回帰を
  引き起こしたため不採用。
- JIT ON は現時点で JIT OFF より遅い (~190 vs ~217 MHz): ExecuteNextJit の NoInlining
...
約 217 MHz (Avg モード実測)、約6.7 サイクル/命令
...
- **Compile Threshold**（デフォルト: 16）: 基本ブロックがコンパイルされるまでに必要な
  ...
  現時点では JIT は全体的にマイナスの性能影響（MHz で約 12.3% 低速）があるため、この値の
  調整による実用的な効果は限定的。
...
- メソッド選択は bool 分岐。Func<bool> デリゲートディスパッチは約 17% の性能回帰を
...
|--------|----------------|------|
| JIT OFF | ~217 | ベースライン |
| JIT ON | ~190 | -12.3% オーバーヘッド |

JIT ON は現時点で JIT OFF より遅い（-12.3% MHz）。ExecuteNextJit の
