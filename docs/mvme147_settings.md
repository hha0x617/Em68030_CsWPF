...
This document describes how the **MVME147** tab in `em68030_CsWPF`'s
**Settings** dialog applies changes to the running emulator. The same
behavior applies to `em68030_WinUI3Cpp` (C++ WinUI 3 standalone) and to
the plugin frontends `emfe_WinUI3Cpp` / `emfe_CsWPF`, since the
settings layer (`EmulatorConfig` + apply logic) is shared across all
four frontends.
