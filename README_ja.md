...
### パフォーマンス
Intel Core i7-13700 上で概算約 217 MHz (サイクルベース推定) のエミュレーション速度を達成。主な最適化:
...
**現状**: 本機能は実験的であり、**デフォルトで無効**です。現在の実装では、JIT を有効にするとエミュレーション速度が約 217 MHz から約 190 MHz に低下します。ExecuteNextJit の NoInlining メソッド呼出オーバーヘッド + block.Execute() の DynamicMethod デリゲートディスパッチコストがコンパイル済みブロックの恩恵を上回っています。コンパイル可能ブロックが実際のコードのごく一部しかカバーしていないことが根本原因です。
...
ビルド済みインストーラは [Releases](https://github.com/hha0x617/Em68030_CsWPF/releases) ページからダウンロードできます。
最新の `Em68030-WPF-Setup-x64-*.exe` をダウンロードして実行してください。
[.NET 8.0 Desktop Runtime](https://dotnet.microsoft.com/download/dotnet/8.0) が必要です。
...
設定ファイルは `%LOCALAPPDATA%\Em68030_CsWPF\` に保存されます。
...
- パフォーマンス: JIT コンパイラの対応命令パターン拡張 (Phase 1 レジスタ命令のみ完了)
