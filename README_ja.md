...
### パフォーマンス
Intel Core i7-13700 上で約 33 MIPS (概算約 204 MHz) のエミュレーション速度を達成。ステータスバーに概算 MHz (サイクルベース) と MIPS (命令スループット) を併記表示します。主な最適化:
...
**現状**: 本機能は実験的であり、**デフォルトで無効**です。現在の実装では、JIT を有効にするとエミュレーション速度が約 33 MIPS から約 31 MIPS に低下します。JIT インフラのオーバーヘッドがコンパイル済みブロックの恩恵を上回っています。対応命令セット (レジスタ間操作のみ) が実際のコードのごく一部しかカバーしていないことが根本原因です。
